...
Hier hebben we een logger aangemaakt voor de module 'my_module' met een eigen logniveau. 
Dit stelt ons in staat om logboekberichten te genereren die specifiek zijn voor deze module.
De berichten worden doorgegeven aan de configuratie die we met `basicConfig()` hebben ingesteld, en krijgen dus hetzelfde logformaat.

WARNING: Voeg je daarnaast nog een eigen handler toe aan de logger (bijvoorbeeld `logger.addHandler(logging.StreamHandler())`), dan verschijnt elk bericht ##twee keer##: één keer via de eigen handler en één keer via de configuratie van `basicConfig()`.

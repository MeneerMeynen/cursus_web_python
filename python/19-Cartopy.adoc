...
[source, python]
----
# Bepaal de extent: enkel (West-)Europa
extent = [-10, 20, 30, 60]
----
...
# Voeg landen toe aan de kaart
ax.add_feature(cartopy.feature.LAND)
ax.add_feature(cartopy.feature.STATES.with_scale('10m'), linestyle='--', edgecolor='blue')
ax.add_feature(cartopy.feature.BORDERS, linestyle=':', edgecolor='red')
----

`STATES` tekent de provincies (of deelstaten). Standaard gebruikt Cartopy de grove schaal `110m`. Die bevat enkel voor enkele grote landen (zoals de VS) deelstaatgrenzen. Met `.with_scale('10m')` gebruik je de gedetailleerde kaartgegevens, waarin ook de Belgische provincies zitten. De eerste keer worden die gegevens gedownload.

...
Python wordt beschouwd als een ##geïnterpreteerde taal##. De interpreter zet je code ##niet om naar machinecode##, maar naar bytecode, die daarna door de Python Virtual Machine ##instructie per instructie## wordt uitgevoerd. Dit in tegenstelling tot talen zoals C of C++, waarbij de code vooraf volledig wordt omgezet in machinecode door een **compilatieproces**, en je een uitvoerbaar bestand (_executable_) krijgt.

Java zit daartussenin: Java-code wordt vooraf gecompileerd naar bytecode, die vervolgens door de Java Virtual Machine (JVM) wordt uitgevoerd. Dat lijkt dus sterk op de werking van Python.
...
. Vertalen: De geparste code wordt omgezet in **tussentijdse bytecode**. Dit is een compact binair formaat dat de Python Virtual Machine (niet de CPU zelf!) begrijpt en dat sneller kan worden uitgevoerd dan de originele broncode.

. Uitvoering: De bytecode wordt uitgevoerd door de **Python Virtual Machine**. Tijdens deze stap worden variabelen gemaakt, waarden toegewezen, bewerkingen uitgevoerd en resultaten gegenereerd.

De eerste drie stappen gebeuren voor het ##volledige bestand##, nog voor er ook maar één regel wordt uitgevoerd. Pas daarna start de uitvoering (stap 4).

Dat merk je wanneer er een syntaxfout in je bestand staat. Bekijk dit programma:

[source,python]
----
print("start")
x = 1
if x > 0         # de dubbele punt ontbreekt
    print("x is positief")
----
...
Zelfs `print("start")` op regel 1 wordt niet uitgevoerd. Python leest en controleert eerst het hele bestand. Pas als dat volledig in orde is, wordt de bytecode uitgevoerd.

Fouten die pas tijdens de uitvoering optreden (bijvoorbeeld delen door nul), zie je wél pas wanneer Python aan die regel komt. De regels daarvoor zijn dan al uitgevoerd.
...
De Just-In-Time (JIT) compiler is een techniek om de uitvoeringssnelheid van code te verbeteren door delen van de code ##tijdens de uitvoering## te compileren naar machinecode.

==== Hoe Werkt een JIT-Compiler?

Normaal wordt de bytecode instructie per instructie geïnterpreteerd door de virtuele machine. Een JIT-compiler houdt bij welke stukken code heel vaak worden uitgevoerd (bijvoorbeeld de binnenkant van een lus die een miljoen keer draait) en zet die tijdens de uitvoering om naar machinecode. Die machinecode kan de CPU rechtstreeks uitvoeren, zonder tussenkomst van de interpreter. Dat is veel sneller.

==== JIT en Python

De standaard Python-interpreter (CPython, de versie die je downloadt van python.org) gebruikt ##standaard géén JIT-compiler##. Sinds Python 3.13 bestaat er een experimentele JIT, maar die staat standaard uit.

Er bestaan wel andere Python-interpreters met een ingebouwde JIT-compiler, zoals **PyPy**. Die voeren dezelfde Python-code uit, vaak een stuk sneller. Java gebruikt in de JVM trouwens ook standaard een JIT-compiler.
...
In dit voorbeeld draait de lus in `simulate_movement` een miljoen keer. De gewone CPython-interpreter interpreteert elke iteratie opnieuw. Een interpreter met een JIT-compiler (zoals PyPy) merkt dat deze lus heel vaak wordt uitgevoerd en zet ze om naar efficiënte machinecode.

Dit proces gebeurt automatisch, hier heb je als programmeur geen invloed op.

==== Voordelen in Gaming

In de context van gaming kan een JIT-compiler de prestaties van Python-code verbeteren, vooral voor onderdelen van het spel die veelvuldig worden gebruikt, zoals fysica-simulaties, beeldverwerking of AI-berekeningen. Dit kan leiden tot soepelere en responsievere spelervaringen.
...
Python's interpretatieproces biedt voordelen zoals **directe feedback** en gemakkelijke debugging. Echter, omdat elke bytecode-instructie wordt geïnterpreteerd wanneer ze wordt uitgevoerd, kan Python trager zijn dan gecompileerde talen voor intensieve berekeningen in games.
...
Het pad van Python-code naar uitvoering op de CPU omvat diverse tussenstappen, van het omzetten naar bytecode tot het uitvoeren van die bytecode door de Python Virtual Machine. Sommige interpreters (zoals PyPy) gebruiken daarbovenop een JIT-compiler om veelgebruikte code om te zetten naar machinecode.

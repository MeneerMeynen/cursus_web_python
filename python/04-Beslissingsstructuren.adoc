...
In het eerste voorbeeld worden de drie ingesprongen regels enkel uitgevoerd als de voorwaarde waar is. In het tweede voorbeeld hoort enkel de tweede regel bij de `if`: de derde regel wordt altijd uitgevoerd, ongeacht de voorwaarde.

=== Logische Structurering

De juiste indentatie zorgt ervoor dat je code correct wordt uitgevoerd volgens de gewenste logica. Indentatie scheidt codeblokken, zoals loops, functies en conditionele statements, van elkaar. Het bepaalt welke instructies bij welk blok horen, en dus wanneer (en hoe vaak) ze worden uitgevoerd.

NOTE: In tegenstelling tot sommige andere programmeertalen maakt een `if`- of `for`-blok in Python ##géén nieuwe scope## voor variabelen. Een variabele die je binnen een `if` aanmaakt, bestaat ook na de `if` nog. Enkel functies (en klassen) hebben een eigen scope.
...
In het eerste voorbeeld worden beide ingesprongen regels bij elke iteratie van de loop uitgevoerd. In het tweede voorbeeld hoort enkel `print(i)` nog bij de loop: de zin "Dit is een iteratie van de loop." wordt maar één keer afgedrukt, na de voltooiing van de loop.
...
Hier wordt de 'or' operator gebruikt om te controleren of de speler de deur kan openen: Ofwel met een sleutel, ofwel met een code. Hij heeft 1 van de 2 nodig, niet beide.
...
image::images/booleanlogic.png[width=400]

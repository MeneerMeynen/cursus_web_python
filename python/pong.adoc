...
== Wat is pygame?
Pygame is een python library waarmee je eenvoudige 2D-games snel en efficiënt kunt bouwen.
Het wordt veel gebruikt om te leren programmeren en voor kleine (indie)games.
Op de website https://www.pygame.org[pygame.org] vind je documentatie en heel wat voorbeeldprojecten.
...
----
=== Creëer leeg scherm
Nu we pyGame hebben geïnitialiseerd kunnen we een leeg scherm maken waar we onze game op gaan bouwen.
...
Creëer een ´game_manager.py´ bestand waar we de basis op gaan bouwen.
Plaats deze code in de ´game_manager.py´.
...
class GameManager:
    ...
    def main(self):
        ...
        while running:
            ...
            self.clock.tick(self.FPS)
----
<1> Verkrijg de grootte van het scherm.
...
Wanneer je de klasse hebt geïnitialiseerd dan kan je de paddles gaan tekenen.
...
# Add the 2 paddles to the game manager
...
----
<1> Vul het scherm
<2> Teken paddle 1 op het scherm (het scherm wordt als parameter meegegeven)
<3> Teken paddle 2 op het scherm (het scherm wordt als parameter meegegeven)
...
class GameManager:
    ...
    def main(self):
        ...
        while running:
            ...
            # Add key input (buiten de event-lus: de paddles bewegen zolang je de toets inhoudt)
            ...
            self.clock.tick(self.FPS)
----
<1> Als de ´Pijltje naar beneden´-toets wordt ingedrukt dan beweeg paddle1 naar beneden.
<2> Als de ´Pijltje naar boven´-toets wordt ingedrukt dan beweeg paddle1 naar boven.
<3> Als de ´W´-toets wordt ingedrukt dan beweeg paddle2 naar boven.
<4> Als de ´S´-toets wordt ingedrukt dan beweeg paddle2 naar beneden.
...
<2> Teken een witte cirkel met de gegeven x en y coordinaten en de berekende radius (bij een bal van 25 op 25 is dat 12).
...
class GameManager:
    ...
    def main(self):
        ...
        while running:
            ...
            # Lees de ingedrukte toetsen (buiten de event-lus)
            keys = pygame.key.get_pressed()
            if keys[pygame.K_DOWN]:
                self.paddle1.move_down(self.paddle1.velocity, self.screen)
            if keys[pygame.K_UP]:
...
<3> Reset het scherm wanneer de bal ´True´ retourneert.
...
<1> Pak de breedte van de bal en zorg ervoor dat die botst tegen de rand met het uiteinde van de bal en kijk daarna of het tegen de linkse rand botst met coordinaat 0. Daarna reset het scherm. Na de ´or´ zorgt die ervoor dat die rand aan de rechterkant pakt. In de eerste helft wordt de breedte gedeeld door 2 om de straal van de bal te verkrijgen, zodat de rand van de bal (en niet het middelpunt) de botsing bepaalt.
<2> Doe hetzelfde bij ´1´ maar pak de y coordinaat om die te laten botsen met de y as beneden door de ´0´ coordinaat te gebruiken en de bovenste kant van de bal. Na de ´or´ pakt die de hoogte van het scherm zodat die de onderkant van het scherm pakt en die ook gebruikt als rand waar je kunt tegen botsen.
...
class Ball:
    ...
    def move_ball(self, screen, paddle1, paddle2):
        ...
        if self.y - self.height // 2 <= 0 or self.y + self.height // 2 >= screen.get_height():
            self.dy = -self.dy
----
<1> Verkrijg de explosie animatie gebaseerd op het nummer
<2> Vergroot de afbeelding
<3> Toon de animatie op het scherm 1 voor 1 gebaseerd op de locatie van de bal.
<4> Doe dit elke 100 milliseconden
        ...
        while running:
            ...
            if move_ball:
                if move_ball == self.paddle1:
                    # De bal ging links buiten: speler 1 miste, dus speler 2 scoort
                    self.score2 += 1
                elif move_ball == self.paddle2:
                    # De bal ging rechts buiten: speler 2 miste, dus speler 1 scoort
                    self.score1 += 1
                self.reset_screen()
            self.ball.draw_ball(self.screen)

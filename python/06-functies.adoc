...
Functies zijn ##stukken code die een naam hebben gekregen##.
...
NOTE: Een functie die binnen een klasse geschreven is, noemen we een _method_. Klassen en methods komen later in de cursus aan bod.
...
TIP: In dit voorbeeld gebruiken we een **f-string**: een string met een `f` voor de aanhalingstekens. Alles wat tussen accolades `{ }` staat, wordt vervangen door de waarde van die variabele of expressie. `f"Hallo, {naam}!"` geeft dus hetzelfde als `"Hallo, " + naam + "!"`, maar is korter en werkt ook met getallen zonder `str()`: `f"Score: {score}"`.

=== args en kwargs

In sommige gevallen wil je een variabel aantal argumenten aan een functie doorgeven, zonder vooraf te weten hoeveel dat er zullen zijn. De notatie `*args` en `**kwargs` staat dit toe.
...
Het zijn ##de sterretjes `*` en `**` die het werk doen##. De namen `args` en `kwargs` zijn enkel een afspraak: `*getallen` werkt net zo goed.
...
NOTE: In dit hoofdstuk zag je twee stijlen van docstrings: met `:param` en `:return:` (de _reStructuredText_-stijl) en met `Args:` en `Returns:` (de _Google_-stijl). Beide zijn gangbaar. Kies er één en gebruik die consequent in je project.

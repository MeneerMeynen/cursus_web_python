...
- `len(lijst)`: Geeft het aantal elementen in de lijst terug. Let op: dit is geen methode van de lijst, maar een ingebouwde functie. Je schrijft dus `len(getallen)` en niet `getallen.len()`.
...
[source, python]
----
x_coordinaat = speler_positie[0]  # Geeft 100 terug
y_coordinaat = speler_positie[1]  # Geeft 50 terug
----
...
Tupels worden vaak gebruikt in gaming voor gegevens die als één geheel bij elkaar horen en niet stukje per stukje mogen veranderen, zoals een kleur of een vaste startpositie.

[source, python]
----
ROOD = (255, 0, 0)
start_positie = (100, 50)
----

Onveranderlijk betekent dat je ##de inhoud van een tuple niet kan aanpassen##:

[source, python]
----
start_positie[0] = 120   # TypeError: 'tuple' object does not support item assignment
----

De variabele zelf kan je wel een nieuwe tuple geven. Beweegt een speler, dan maak je gewoon een nieuwe tuple voor de nieuwe positie:

[source, python]
----
speler_positie = (100, 50)
speler_positie = (110, 50)   # dit mag: de variabele verwijst nu naar een nieuwe tuple
----
...
- `keys()`: Geeft alle sleutels terug.
- `values()`: Geeft alle waarden terug.
- `items()`: Geeft alle sleutel-waardeparen terug (als tuples).
- `len(dictionary)`: Geeft het aantal sleutels terug. Dit is een ingebouwde functie, geen methode.

`keys()`, `values()` en `items()` geven geen echte lijst terug, maar een _view_ (bijvoorbeeld `dict_keys`). Daar kan je met een `for`-lus over lopen. Heb je echt een lijst nodig, zet de view dan om met `list()`: `list(speler.keys())`.

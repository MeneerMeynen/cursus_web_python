...
Een ruiter is als **klasse** in dit computerspel slechts **eenmaal gedefinieerd**.
...
De ruiter klasse uit bovenstaande _screenshot_ zou als volgt geprogrammeerd kunnen zijn:
...
[IMPORTANT]
====
##De naam van een klasse begint steeds met een hoofdletter##. Dit is geen regel van de taal, maar een afspraak (PEP 8) waar alle Python-programmeurs zich aan houden.
====
...
Een klasse kan in Python ##geen twee methods met dezelfde naam## hebben. Schrijf je toch twee keer een method met dezelfde naam, dan geeft Python ##géén foutmelding##: de tweede definitie overschrijft stilletjes de eerste.
...
Na deze code bestaat enkel nog de tweede `hit()` method (met de parameter `damage`). Roep je `ruiter3.hit()` zonder argument op, dan krijg je een fout:

[source]
----
TypeError: Ruiter.hit() missing 1 required positional argument: 'damage'
----

NOTE: In talen zoals Java kan je wel meerdere methods met dezelfde naam maken, zolang de parameters verschillen (_method overloading_). Python ondersteunt dit niet.

Wil je een method zowel met als zonder argument kunnen oproepen, gebruik dan een ##standaardwaarde## voor de parameter:
...
[source,python]
----
ruiter3.hit(8)   # de health vermindert met 8
ruiter3.hit()    # geen argument: de standaardwaarde 10 wordt gebruikt
----

In de rest van dit hoofdstuk gebruiken we deze versie van de `hit()` method.
...
In Python code zouden we de lijst als volgt kunnen voorstellen:
...
# Een lijst met de geselecteerde Unit-objecten (hier nog leeg)
selected = []
...
In de _standard library_ van Python (die wordt meegeïnstalleerd bij het installeren van Python) zijn veel bestaande modules en klassen aanwezig.
Deze kan je gebruiken zonder ze zelf vanaf 0 te hoeven schrijven.

Een voorbeeld hiervan is de _math_ module.
...
Ook bevat deze module heel wat wiskundige functies, zoals `math.sqrt()` (vierkantswortel), `math.floor()` (afronden naar beneden) en `math.ceil()` (afronden naar boven). Kortom veel zaken die in de wiskunde van pas kunnen komen.

Het maximum en minimum bereken je met de ingebouwde functies `max()` en `min()`, het gemiddelde met `statistics.mean()` uit de `statistics` module.

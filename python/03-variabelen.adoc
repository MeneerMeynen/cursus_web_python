...
Wiskundige en logische expressies kun je samen gebruiken. Python rekent eerst de wiskundige delen uit en maakt daarna de vergelijking:
...
De vier basisdatatypes in Python, `int`, `float`, `str` en `bool`, vormen de bouwstenen van elk programma. Met **wiskundige expressies** reken je met getallen, met **logische expressies** vergelijk je waarden en krijg je `True` of `False`. Hoe je meerdere voorwaarden combineert met `and`, `or` en `not`, zie je in hoofdstuk 4. Deze expressies heb je nodig in elk volgend hoofdstuk: als voorwaarde in beslissingsstructuren en lussen, en om berekeningen te maken in je programma's.

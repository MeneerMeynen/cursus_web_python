...
* Geldigheid: Variabelen moeten met een letter of een underscore (_) beginnen. Ze kunnen ook cijfers (0-9) bevatten, maar niet als eerste teken. Bijvoorbeeld: player_score, _health, level1. Python laat ook letters met accenten toe (zoals `ë` of `ö`), maar de afspraak is om enkel a-z, A-Z, cijfers en underscores te gebruiken.
...
Het gegevenstype `int` staat voor gehele getallen, zoals 5, -10 en 100. Integer-waarden worden gebruikt voor wiskundige bewerkingen en numerieke berekeningen. Een `int` kan positief, negatief of nul zijn, maar heeft nooit een decimaal deel. Zodra een getal een decimaal punt bevat, zoals `5.0`, is het een `float`. Let op: Python gebruikt altijd een **punt** als decimaal scheidingsteken, nooit een komma. Voor grote getallen mag je wel een underscore gebruiken om ze leesbaarder te maken: `1_000_000`.
...
Het gegevenstype `float` vertegenwoordigt komma-getallen, ook wel bekend als zwevendekomma-getallen. Dit omvat getallen met decimale punten, zoals 3.14, -0.5 en 2.0. Float-waarden worden gebruikt voor berekeningen met reële getallen.
...
WARNING: Een float is een ##benadering##. De computer slaat kommagetallen binair op, waardoor sommige getallen niet exact kunnen worden voorgesteld. Zo geeft `print(0.1 + 0.2)` als resultaat `0.30000000000000004`. Wil je een net resultaat tonen, rond dan af: `round(0.1 + 0.2, 2)` geeft `0.3`.
...
== Type Conversie en Dynamische Typing

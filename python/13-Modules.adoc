...
# Voorbeeld 2: Afronden naar beneden en naar boven
getal_met_decimaal = 4.56
print(f"Naar beneden: {math.floor(getal_met_decimaal)}")  # 4
print(f"Naar boven: {math.ceil(getal_met_decimaal)}")     # 5
...
Laten we bijvoorbeeld de populaire `matplotlib`-module installeren, die wordt gebruikt om grafieken te maken (zie het hoofdstuk over Matplotlib).
...
Hiermee wordt versie 3.8.0 van de `matplotlib`-module geïnstalleerd.

...
= Hoofdstuk 8: Gegevensinvoer en Bestanden

In dit hoofdstuk gaan we in op gegevensinvoer: invoer van de gebruiker via het toetsenbord, en het lezen en schrijven van bestanden. We combineren de invoer met de expressies uit hoofdstuk 3.
...
----
<1> `input()` geeft altijd een string (`str`) terug, ook als de gebruiker een getal intypt. Daarom zetten we de invoer hier om naar een integer met `int()`. Zo kan ermee gerekend worden, met een `str` zou dat niet kunnen.
...
TIP: Geef bij het openen van tekstbestanden best ook de codering mee: `open('voorbeeld.txt', 'w', encoding='utf-8')`. Zonder `encoding` gebruikt Python de standaardcodering van je besturingssysteem. Op Windows kunnen letters zoals é, ë of ç dan verkeerd worden gelezen of weggeschreven.
...
Hier wordt elke regel van het bestand afzonderlijk gelezen en afgedrukt. Elke gelezen regel eindigt met een `\n` (nieuwe regel), en `print()` voegt zelf ook een nieuwe regel toe. Zonder `.strip()` zou er dus telkens een lege regel tussen staan. `.strip()` verwijdert witruimte en `\n` aan het begin en einde van de regel.
...
* **Bestand hernoemen:** Gebruik de `os.rename()`-functie.
* **Bestand verplaatsen:** Gebruik de `shutil.move()`-functie uit de `shutil`-module.
* **Bestand kopiëren:** Gebruik de `shutil.copy()`-functie uit de `shutil`-module. (De `os`-module heeft geen functie om bestanden te kopiëren.)

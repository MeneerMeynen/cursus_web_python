...
Een csv bestand bevat verschillende gegevens die met een komma gescheiden zijn (vandaar csv: __Comma Separated Values__).
...
Filteren doen we door voor elke row een bepaalde key aan de dictionary te vragen:
...
Deze temperatuur kunnen we omzetten in Celsius. Hiervoor maken we zelf een functie aan:

[source, python]
----
def FtoC(F):
    '''Deze functie zet de temperatuur van Fahrenheit om naar Celsius'''
...
temperatures.append(celsius) # telkens bij het uitlezen van de row
----
    ...
    with open("object-oriented-programming\\Modules\\data_plotting\\city_temperature.csv", "r") as csvfile:
        ...
        for row in reader:
            ...
            if row["City"] == "Brussels" and row["Year"] == year:
                ...
                fahrenheit = row["AvgTemperature"]
                # Zet de temperatuur om naar graden Celsius
                celsius = FtoC(fahrenheit)
                # Meetfouten halen we uit de gegevens (ontbrekende metingen staan in het bronbestand als -99 °F)
                if celsius < -30:
                    # Bij een meetfout slaan we deze rij over en gaan we naar de volgende iteratie van de lus
                    continue
                # Voeg de gevonden temperatuur toe aan de lijst van temperaturen
                temperatures.append(celsius) # F° => C°

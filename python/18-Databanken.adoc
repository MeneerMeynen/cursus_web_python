...
Dit geeft alle records en kolommen van de 'artist'-tabel weer:
...
[source, sql]
----
-- Selecteer artiesten met een specifieke naam uit de 'artist' tabel
...
SELECT AlbumId, COUNT(TrackId) AS AantalTracks FROM track GROUP BY AlbumId;
----
...
In de 'artist'-tabel van de Chinook-database is het veld 'ArtistId' een voorbeeld van een Primary Key. Hiermee wordt elke artiest #uniek geïdentificeerd#. Een bepaald primary key nummer kan dus maar 1 keer voorkomen in een tabel.

.Voorbeeld van een Primary Key in de 'artist'-tabel
|===
| ArtistId | Name
| 1        | AC/DC
| 2        | Accept
...
In de 'album'-tabel van de Chinook-database verwijst het veld 'ArtistId' naar de Primary Key 'ArtistId' in de 'artist'-tabel. Hiermee wordt een relatie tussen de twee tabellen gevestigd.

.Voorbeeld van een Foreign Key in de 'album'-tabel
|===
| AlbumId | Title                         | ArtistId
...
In dit voorbeeld verwijst het veld 'ArtistId' naar de Primary Key 'ArtistId' in de 'artist'-tabel, waardoor een **relatie ontstaat** tussen de artiesten en de albums in de database.
...
Bij het verwijderen van gegevens uit de databank moet er dus steeds opgelet worden dat je niets verwijdert waarnaar een FK verwijst. De referentie (FK naar PK) mag niet verstoord worden. Voor het verwijderen zal je de FK-PK verwijzing eerst moeten opheffen of verwijderen. Dit is de referentiële integriteit.

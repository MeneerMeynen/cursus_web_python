...
+
* ARM, een zuinigere architectuur. Deze zit in bijna iedere smartphone en in de nieuwere Macs.
+
image::images/arm.webp[ARM cpu,width=30%, height=auto]
+
* SPARC, een architectuur van Sun die vroeger veel in servers werd gebruikt
+
image::images/KL_Sun_UltraSparc.jpg[SUN Sparc RISC cpu,width=30%, height=auto]
+
* z/Architecture, de architectuur van de mainframes van IBM (grote, zeer betrouwbare computers voor banken en overheden)
+
...
ARM en SPARC zijn zogenaamde **RISC**-architecturen (_Reduced Instruction Set Computer_): ze werken met een kleine set eenvoudige instructies. x86 en x64 hebben een veel grotere en complexere instructieset.
...
Iedere bestaande taal, zoals Python, wordt ook verder doorontwikkeld. Het verschil tussen python versie 1 en versie 3.11 is enorm groot. Hoewel het om dezelfde programmeertaal gaat, zal een programma ontwikkeld voor python 3, niet meer werken op python 1.
...
- **1980s-1990s:** Objectgeoriënteerde talen zoals C++ en Java versterkten concepten zoals herbruikbaarheid en complexe softwareontwikkeling. In 1991 verscheen ook de eerste versie van Python, met de nadruk op leesbaarheid en eenvoud.
- **2000s-heden:** Talen zoals Python en JavaScript worden enorm populair, en nieuwe talen zoals C#, Go en Rust verschijnen. De nadruk ligt op leesbaarheid, productiviteit, veiligheid en veelzijdigheid.
